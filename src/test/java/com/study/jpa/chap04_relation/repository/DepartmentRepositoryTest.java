...
class DepartmentRepositoryTest {
    ...
    // 영속성 컨텍스트 내에 내용들을 DB에 반영시키거나 비워내거나 수명을 관리할 수 있는 객체.
    // 특정시점에서 save를 강제 커밋 가능
    ...
    void testChangeDept() {
        ...
        newDept.getEmployees().add(foundEmp); // 2번 방법 때 사용하려고 새로 만듦



        employeeRepository.save(foundEmp); // 트랜잭션이 끝나야 세이브된다 코드가 돌 때는 아직 세이브 되지 않음

        // 1번 방법
        // 변경 감지( 더티 체크 ) 후 변경된 내용을 DB에 적시 반영하는 역할
