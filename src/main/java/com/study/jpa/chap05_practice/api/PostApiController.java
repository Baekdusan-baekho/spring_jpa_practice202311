...
public class PostApiController {
    ...
    /*
        ...
        게시물 등록:     /posts            - POST, payload: (writer, title, hashTags)
        게시물 수정:     /posts            - PUT(아예 통째로 갈아끼움), PATCH(내부를 수정함), payload: (title, content, postNo)
        게시물 삭제:     /posts/{id}       - DELETE
     */
    private final PostService postService;

    @GetMapping // 게시물 목록 조회
    public ResponseEntity<?> list(PageDTO pageDTO) {log.info("/api/v1/posts?page={}&size={}", pageDTO.getPage(), pageDTO.getSize());
    ...
    public ResponseEntity<?> detail(@PathVariable Long id){
        ...
        try {
            ...
            // 화면단에 잘못된 요청을 보냈다고 body에 예외의 매개값을 전달한다.
        }
